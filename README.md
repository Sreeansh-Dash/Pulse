...
Furthermore, to control the throughput of concurrent network requests and avoid exhausting local resources or hitting remote rate-limits, the application leverages an **`asyncio.Semaphore()`** (configurable via the `MAX_CONCURRENT_CHECKS` environment variable). While the Lock protects shared state from race conditions during writes, the Semaphore strictly governs application-level concurrency limits before requests ever touch the network layer—demonstrating the deliberate use of two distinct synchronization primitives for separate purposes (state protection vs. throughput control).
